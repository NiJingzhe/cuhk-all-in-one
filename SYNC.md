...
1. **环境自检**：`gh auth status` 可用（不可用 → 如实告知用户"本会话无法开 PR"，后续分享流程相应降级）；工作区必须干净（有未提交改动 → 属于 feat 分支的先提交，其余按「不愿分享」流程落 personal 后 `git restore`）。
2. `git fetch origin`，将本地 main 快进到 `origin/main`。
3. 读取 `personal/prs.md` 中登记的**每个**未结 PR：
