...
本技能的私有层**不在仓库/skill 目录里**，而在用户主目录下，任何安装与更新都碰不到它：

- macOS / Linux：`~/.cuhk/personal`
- Windows：`%USERPROFILE%\.cuhk\personal`

下文统一简写为 `~/.cuhk/personal`。读写它之前先按当前系统换算真实路径。

## 会话启动协议（Bootstrap —— 硬性前置，任何知识回答之前必须跑完）

> 本节在 SKILL.md 被加载的那一刻生效，**与用户问什么无关**：哪怕只是问一句"校巴几点"，也要先完成下面的检查。不设例外的理由：跳过的代价不在当下，而在会话后段才爆出来——到要开 PR 时才发现 gh 没登录、prs.md 里的 PR 忘了维护、main 落后导致引用了过期知识、脏 diff 混进下一次提交。整个流程只是几条秒级命令，不构成省略的理由。

按顺序执行；**任何一步失败必须当场向用户报告，禁止静默略过**：

1. **初次使用检测**：`~/.cuhk/personal/info.md` 是否存在？不存在 → 运行 `scripts/personal-init.sh`（Windows 用 `scripts\personal-init.bat`，本目录即 skill 根）建立脚手架，然后按 `SYNC.md`「首次使用」补齐一次性流程（询问学位层次与专业填 info.md → 询问是否停用其他学科路由）。若发现 skill 目录里残留旧版 `personal/` 目录，按 SYNC.md「旧版迁移」并入新路径后再继续。
2. **git 化与同步**：运行 `scripts/git-setup.sh`（Windows 用 `scripts\git-setup.bat`）。它幂等：`npx skills add` 装出的副本没有 `.git`，会被原地初始化为指向官方仓库的 git 仓库（fork/PR 由此可用）；随后 `git fetch` + `merge --ff-only` 同步上游。**只动 `.git`，不动工作区文件**。失败 → 明确告知用户"本次未同步，本地知识可能落后于 main"。**禁止改用 `npx skills update` 同步**——它整目录重刷，会连 `.git` 和未分享的改动一起清掉。
3. **维护未结 PR**：读 `~/.cuhk/personal/prs.md` 的每条记录——已 merged/closed → 删本地 feat 分支并移除该条；仍 open → 切分支 `merge origin/main` 后 push，再回到 main。
4. **环境自检**：`gh auth status` 可用（不可用 → 如实告知"本会话无法开 PR"，之后的分享流程相应降级）；`git status` 必须干净（有脏 diff 先归位：属于 feat 分支的提交上去，其余按 `SYNC.md`「不愿分享」落 personal 后 `git restore . && git clean -fd knowledge/`）。
5. **新鲜度警觉**：当前日期临近学期切换点（1 月 / 9 月），或将引用文档的 `freshness.stable_until` 已过 → 回答前先按该文档 `freshness.check` 复核（规范见「新鲜度协议」节）。

**知识加载铁律**：`~/.cuhk/personal/` 下存在同相对路径文件时，**一律以 personal 为准**（如 `~/.cuhk/personal/knowledge/common/dining/x.md` 覆盖 `knowledge/common/dining/x.md`）。

**会话结束前**：新产生的知识问用户是否分享——愿意则开 `feat/*` 分支 + 结构化 PR（标题带 UTC 时间戳）并登记进 prs.md；不愿意则把新增/修改文件落到 `~/.cuhk/personal/` 的**同相对路径（与仓库根同构，如 `knowledge/common/dining/x.md` → `~/.cuhk/personal/knowledge/common/dining/x.md`）**，然后 `git restore . && git clean -fd knowledge/` 还原 skill 目录。
